...
> **仅供授权渗透测试、安全研究与 CTF 竞赛使用。未经授权使用本工具集访问或破坏任何系统均属违法行为，后果由使用者自行承担。**

这是一个面向 Linux 本地提权场景的工具集合，整理了常见内核 / SUID / sudo / polkit / PackageKit 漏洞 PoC，以及信息收集、密码字典、反弹 Shell 和辅助脚本。README 的目标是让你快速知道：这里有什么、各文件放在哪、在什么场景下该用哪一类工具。
...
- [提权漏洞 PoC](#提权漏洞-poc)
  ...
  - [07dirtyfrag](#07dirtyfrag)
  - [08fragnesia](#08fragnesia)
  - [09key-sign-pwn](#09key-sign-pwn)
- [信息收集工具](#信息收集工具)
...
| 类别 | 位置 | 说明 |
|------|------|------|
| 提权 PoC | `01copyfail/` | CVE-2026-31431，内核页缓存写原语利用 |
| 提权 PoC | `02dirtypipe/root` | CVE-2022-0847，预编译 Dirty Pipe ELF |
| 提权 PoC | `03pwnkit/` | CVE-2021-4034，polkit pkexec 利用 |
| 提权 PoC | `04chwoot/` | CVE-2025-32463，sudo `-R` 路径穿越 |
| 提权 PoC | `05pack2theroot/` | CVE-2026-41651，PackageKit TOCTOU |
| 提权 PoC | `06sudo3165/` | CVE-2021-3156，sudo 堆溢出 |
| 进阶 PoC | `07dirtyfrag/` | Dirty Pipe / frag 相关二进制 |
| 进阶 PoC | `08fragnesia/` | fragnesia 演示与源码 |
| 进阶 PoC | `09key-sign-pwn/` | ssh-keysign / chage 相关提权链 |
| 信息收集 | `lin2026.sh` / `linpeas.sh` | Linux 本地枚举脚本 |
| 进程监控 | `pspy32` / `pspy64` / `pspy64s` | 无需 root 的进程监控 |
| 辅助工具 | `busybox` / `socat` | 静态 BusyBox、端口转发与 Shell 工具 |
...
| su 爆破 | `subrute.sh` | 基于字典的本地 su 爆破 |
...
│   └── fragnesia.c            # 源码
├── 09key-sign-pwn/
│   └── ssh-keysign-pwn/       # ssh-keysign / chage 相关 PoC
...
1. 先确认目标环境，再选择对应目录，不要直接盲跑所有 PoC。
2. 优先做无副作用检查，例如内核版本、sudo 版本、pkexec 是否存在、PackageKit 版本是否受影响。
3. 先用 `pspy`、`linpeas`、字典和辅助工具完成信息收集，再决定是否进入漏洞利用链。
4. 仅在授权目标和隔离靶场中使用这些工具。
...
[LinPEAS 官方仓库](https://github.com/peass-ng/PEASS-ng) — Linux 本地权限提升枚举脚本，自动检查数百项安全配置项，包括：
